...
export default function RoomSetup({ onJoin, onCreate }: RoomSetupProps) {
  ...
  return (
    <div className="min-h-dvh flex items-center justify-center p-6 relative overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-sm animate-fade-in glass-strong rounded-2xl p-8">
        ...
        {mode === 'select' && (
          <div className="space-y-3">
            <button
              ...
              className="w-full py-3 rounded-xl bg-indigo-500 hover:bg-indigo-400 text-white font-semibold text-sm transition-all shadow-lg shadow-indigo-500/20"
            >
              Create New Room
            </button>
            <button
              onClick={() => setMode('join')}
              className="w-full py-3 rounded-xl bg-dark-700 hover:bg-dark-600 font-semibold text-sm transition-all"
            >
              Join Existing Room
            </button>
          </div>
        ...
        {(mode === 'create' || mode === 'join') && (
          <form onSubmit={mode === 'create' ? handleCreate : handleJoin} className="space-y-4">
            ...
            <button type="submit" className="w-full py-3 rounded-xl bg-gradient-to-r from-indigo-500 to-purple-500 text-white font-semibold text-sm transition-all shadow-lg hover:shadow-indigo-400/30">
              {mode === 'create' ? 'Create Room' : 'Join Room'}
            </button>
            
            <button type="button" onClick={() => setMode('select')} className="w-full py-2 text-dark-400 hover:text-dark-200 text-sm mt-2 transition-colors">
              Back
            </button>
          </form>
        )}
